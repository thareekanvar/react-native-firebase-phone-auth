...
jest.mock('react-native/Libraries/Core/Devtools/symbolicateStackTrace', () => jest.fn(async (stack) => 
...
it(`serializes simple log messages`, async () => {
    let result = await LogSerialization.serializeLogDataAsync(['hi', 1, true, 0, '', null, false, undefined], 'info');
...
it(`serializes nested objects`, async () => {
    let result = await LogSerialization.serializeLogDataAsync([{ outer: { inner: [[], {}] } }], 'info');
...
it(`serializes symbols`, async () => {
    let result = await LogSerialization.serializeLogDataAsync([Symbol('test'), Symbol.iterator], 'info');
    expect(result.body).toEqual(['Symbol(test)', 'Symbol(Symbol.iterator)']);
    expect(result.includesStack).toBeFalsy();
});
it(`serializes promises`, async () => {
    let result = await LogSerialization.serializeLogDataAsync([Promise.resolve('test'), Promise.reject(new Error('Expected'))], 'info');
...
it(`serializes React components (refs)`, async () => {
    ...
    let result = await LogSerialization.serializeLogDataAsync([testRenderer.root.instance.child], 'info');
    expect(Array.isArray(result.body)).toBe(true);
...
//# sourceMappingURL=LogSerialization-test.js.map