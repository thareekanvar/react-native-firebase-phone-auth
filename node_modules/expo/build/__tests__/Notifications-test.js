...
describe('Notifications', () => {
    ...
    it('properly passes time as mumber when scheduling notification on ios', async () => {
        ...
        expect(spy).toHaveBeenCalledWith({ data: {}, ...mockedScheduledNotifIOS }, { repeat: 'minute', time: notifDate.getTime() });
    });
    it('properly passes time as Date when scheduling notification on Android', async () => {
        ...
        NativeModules.ExponentNotifications.scheduleLocalNotification = spy;
        const notifDate = new Date();
        await Notifications.scheduleLocalNotificationAsync({ title: 'Android notification' }, {
            // we pass time as number, but below it should be passed as date
            time: notifDate.getTime(),
            repeat: 'minute',
        });
        expect(spy).toHaveBeenCalledTimes(1);
        expect(spy).toHaveBeenCalledWith({ data: {}, title: 'Android notification' }, { repeat: 'minute', time: notifDate });
    });
    it('properly passes "options.intervalMs" when scheduling notification on android', async () => {
        ...
        expect(spy).toHaveBeenCalledTimes(1);
        expect(spy).toHaveBeenCalledWith({ data: {}, title: 'Android notification' }, { intervalMs: 1000, time: notifDate });
    });
    it('properly detects invalid time value in scheduled notification options', async () => {
        ...
        try {
            ...
            expect(e).toEqual(new Error(`Provided value for "time" is invalid. Please verify that it's either \
...
a number representing Unix Epoch time in milliseconds, or a valid date object.`));
    ...
    it('properly warns when time value prior to now is used in scheduled notification options', async () => {
        ...
        expect(warning).toBeCalledWith(false, `Provided value for "time" is before the current date. Did you possibly \
...
function emitNativeNotification(notif) {
    require('react-native/Libraries/EventEmitter/RCTDeviceEventEmitter').emit('Exponent.notification', notif);
}
