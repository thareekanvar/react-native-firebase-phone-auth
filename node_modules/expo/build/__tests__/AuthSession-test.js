...
it('returns the correct return URL from getDefaultReturnUrl', () => {
    expect(AuthSession.getDefaultReturnUrl()).toEqual('exp://exp.host/@test/test/--/expo-auth-session');
});
it('returns the correct start URL from getStartUrl', () => {
    ...
    expect(result).toEqual('https://auth.expo.io/@example/abc/start?authUrl=https%3A%2F%2Fsignin.com&returnUrl=exp%3A%2F%2Fexpo.io%2F%40example%2Fabc%2B');
});
it('opens WebBrowser startAsync to the start URL', async () => {
    ...
    expect(WebBrowser.openAuthSessionAsync).toHaveBeenCalledWith(AuthSession.getStartUrl(authUrl, returnUrl), returnUrl);
});
